...
function MycartAdd(){
    ...
    //var pctps = document.getElementsByName('mycheckbox');
    //取得数组
   // var pctps = $("input:checkbox[name='mycheckbox']:checked").val();   
    $.ajax({
        url: "tools/submit_ajax.ashx?action=mycart_goods_add",
        data: {
            "article_id": articleId,
            "quantity": selectNum,
            "pctps": selected_value
        },
        //data: _list,  
        dataType: "json",
        type: "POST",
        ...
        success: function (responseJSON) {
            ...
            if (responseJSON.status == 1) {
                ...
                $("#shoppingCartCount").text(responseJSON.Count);
...
function Pay(p_id) {
    ...
    $.ajax({
        ...
        success: function (responseJSON) {
            // your logic  
            if (responseJSON.status ==1) {                        
                window.location.href = "lsfs.aspx?gouwuche_id=" + responseJSON.gouwuche_id + "&p_id=" + p_id;
                $("#shoppingCartCount").text(responseJSON.Count);
            }
            else if (responseJSON.status ==3) {
                window.location.href = "login.aspx";
